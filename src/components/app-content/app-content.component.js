...
import {ReactAst,AngularAst,PolymerAst} from './../../utils/ast';
    ...
    parseGithub() {
        ...
        this.githubReaderService.getTreeRecursively(this.gitHubOwner, this.gitHubRepo, this.gitHubSha, this.folderToParse)
            .then(function (container) {
                ...
                currentClass.githubRepositoryContainer = container;
                currentClass.githubFolderStructure = container.getFolderStructure();

                if (container.tree.length === 0) {
                    // we found no files
                    currentClass.loading = false;
                ...
                for (let i = 0; i < container.tree.length; i++) {
                    ...
                    try {
                        ...
                        console.warn('[parseGithub] could not process file', fileEntry.path, e);
                        currentClass.addOutputLog('Unable to process file '+fileEntry.path, 'error');
                        continue;
                    }
                    ...
                    modelExtractorChain.getProcessErrors().forEach(a => {
                        ...
                        currentClass.addOutputLog('Failure to process the file '+fileEntry.path+' with the '+a.extractor);
                    });
                }
                // display the output of the semantic analyser & information extraction
                currentClass.syntaxContent = JSON.stringify(extractedAstContent, null, '\t');
                currentClass.tokensContent = JSON.stringify(extractedTokenContent, null, '\t');
                currentClass.infoBaseContent = JSON.stringify(extractedInfoBase, null, '\t');
                ...
                if (Object.keys(extractedInfoBase).length > 0) {
                    ...
                    currentClass.modelContent = JSON.stringify(generatedModel, null, '\t');

                    currentClass.dependencyGraph = JSON.stringify(
